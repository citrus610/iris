...
i32 Engine::pvsearch(Data& data, i32 alpha, i32 beta, i32 depth, bool is_cut)
{
    ...
    while (true)
    {
        ...
        if (legals > 1 + is_root * 2 &&
            ...
            i32 depth_reduced = std::clamp(depth_next - reduction, 1, depth_next) + is_pv;
        ...
        if (score >= beta) {
            ...
            const i16 bonus = history::get_bonus(depth + (eval_static < best)) - is_cut * tune::HS_BONUS_CUT_COEF;
