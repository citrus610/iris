...
i32 Engine::aspiration_window(Data& data, i32 depth, i32 score_old)
{
    ...
    {
        // Principle variation search
        score = this->pvsearch<Node::ROOT>(data, alpha, beta, depth, false);
...
i32 Engine::pvsearch(Data& data, i32 alpha, i32 beta, i32 depth, bool is_cut)
{
    ...
    if (!is_pv &&
        ...
        // Scouts
        i32 score = -this->pvsearch<Node::NORMAL>(data, -beta, -beta + 1, depth - reduction, false);
    ...
    while (true)
    {
        ...
        if (!is_root &&
            ...
            // Search
            i32 score = this->pvsearch<Node::NORMAL>(data, singular_beta - 1, singular_beta, singular_depth, is_cut);
        ...
        if (legals > 1 + is_root * 2 &&
            ...
            // Scouts
            score = -this->pvsearch<Node::NORMAL>(data, -alpha - 1, -alpha, depth_reduced, true);

            // Failed
            if (score > alpha && depth_reduced < depth_next) {
                score = -this->pvsearch<Node::NORMAL>(data, -alpha - 1, -alpha, depth_next, !is_cut);
            }
        }
        ...
        if (is_pv && (legals == 1 || score > alpha)) {
            score = -this->pvsearch<Node::PV>(data, -beta, -alpha, depth_next, false);
        }
...
template i32 Engine::pvsearch<Node::ROOT>(Data&, i32, i32, i32, bool);
template i32 Engine::pvsearch<Node::PV>(Data&, i32, i32, i32, bool);
template i32 Engine::pvsearch<Node::NORMAL>(Data&, i32, i32, i32, bool);
