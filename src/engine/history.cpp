...
i16& Table::get(i8 color, const u64& threats, const u16& move)
{
    assert(square::is_valid(move::get_from(move)));
    assert(square::is_valid(move::get_to(move)));

    const bool is_threaten_from = threats & bitboard::create(move::get_from(move));
    const bool is_threaten_to = threats & bitboard::create(move::get_to(move));

    return this->data[color][move & 0xFFF][is_threaten_from][is_threaten_to];
};

void Table::update(i8 color, const u64& threats, const u16& move, i16 bonus)
{
    history::update<history::quiet::MAX>(this->get(color, threats, move), bonus);
};
